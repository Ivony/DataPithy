...
namespace Ivony.Data.Test
{
  ...
  public class ParameterizedQueryTest
  {
    ...
    public void TemplateParserTest()
    {
      ...
      var query = DbEnv.Default.T( $"SELECT * FROM Users" );
      Assert.AreEqual( query.TextTemplate, "SELECT * FROM Users", "无参数模板解析测试失败" );

      query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID = '#123'" );
      ...
      query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID = '{{0}}'" );
      Assert.AreEqual( query.TextTemplate, "SELECT * FROM Users WHERE ID = '{0}'", "花括号转义测试失败" );

      query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID = {{{1}}}" );
      ...
      query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID = ( {$"SELECT ID FROM Users WHERE Username = {"Ivony"}" } )" );
      ...
      query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID = ( {$"SELECT ID FROM Users WHERE Username = {"Ivony"}" } ) AND Status = {3}" );
    ...
    {
      var query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID IN ( {(1, 2, 3)} )" );
      ...
      query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID IN ( {ValueTuple.Create()} )" );
      ...
      query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID IN ( {("1", "2", "3")} )" );
    ...
    {
      var query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID = {null} AND Username = {null}" );
      ...
      query = DbEnv.Default.T( $"SELECT * FROM Users WHERE ID = {null}" );
    ...
    public void ConcatTest()
    {
      ...
      query = DbEnv.Default.T( $"SELECT * FROM Users WHERE UserID = {1};" );
    ...
    public void JoinTest()
    {
      var query = DbEnv.Default.T( $"SELECT * FROM Users" );
      query += "WHERE";
      query += DbEnv.Default.Join( "AND", new[] { DbEnv.Default.T( "UserID <> {0}", 0 ), DbEnv.Default.T( "Username = {0}", "Ivony" ) } );
      ...
      query = DbEnv.Default.T( "SELECT * FROM Users" );
      query += "WHERE";
      query += DbEnv.Default.Join( "AND", new ParameterizedQuery[0] );
      ...
      query = DbEnv.Default.T( "SELECT * FROM Users" );
      query += "WHERE";
      query += DbEnv.Default.Join( "AND", new ParameterizedQuery[] { null } );
      ...
      query = DbEnv.Default.T( "SELECT * FROM Users" );
      query += "WHERE";
      query += DbEnv.Default.Join( "AND", new ParameterizedQuery[] { DbEnv.Default.T( "UserID <> {0}", 0 ), null, DbEnv.Default.T( "Username = {0}", "Ivony" ) } );
